...
bool CoHandle::RayCast(vec3 const& ray_start, vec3 const& ray_end)
{
	...
	for (int32 h_idx = 0; h_idx < num_handle; h_idx++)
	{
        ...
        transform handle_transform(world_line_handle_quat, world_line_handle_pos, cube_size);
        
        vec3 ray_start_box = handle_transform.TransformPositionInverse(ray_start);
        vec3 ray_end_box = handle_transform.TransformPositionInverse(ray_end);
        ...
        dist = intersect::RaySphereIntersection(ray_start_box, ray_dir_box, vec3(0.f, 0.f, 0.f), cube_size * 0.85f);
	}
#endif
