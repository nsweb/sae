...
#include "common/dftools.h"
    ...
    uniform vec3 scene_center = vec3( 0., 0., 0.f );
#endif
...
vec2 map_ext(vec3 pos, vec3 dir)
{
    ...
    float l = (scene_center.z - pos.z) / dir.z;
    vec2 inter = pos.xy + dir.xy * l;
    vec2 v = inter - scene_center.xy;
    float v2 = dot(v, v);
    if( v2 > 40.*40. )
        return vec2( -1., -1. );   // bg
...
void main(void)
{
    ...
    else
    {
        ...
        if( res.y < 1.5 )
        {
            ...
            col = mix( col, vec3(1., 0., 0.), smoothstep(0.02, 0., abs(pos.y)) );
            col = mix( col, vec3(0., 1., 0.), smoothstep(0.02, 0., abs(pos.x)) );
        }
